...
public class RSAKey extends Key implements JSONEncoder {
  ...
  /**
   * Hardcoded method string. used to compute the keyid and to indicate which signing mechanism was
   * used to compute this signature.
   */
  private final String scheme = "rsassa-pss-sha256";

  /**
   * Hardcoded hashing algorithms. used to compute the keyid, as well as to indicate which hash
   * algorithms can be used to compute the keyid itself.
   */
  private final String[] keyid_hash_algorithms = {"sha256", "sha512"};

  /**
   * Hardcoded keytype. This field exists for backwards compatibility, as the scheme field is more
   * descriptive.
   */
   ...
   *
   * <p>You most likely want to use the static method {@link #read read} to instantiate this class.
   *
  ...
  public AsymmetricKeyParameter getPrivate() throws IOException {
    if (this.kpr == null) return null;
    if (this.kpr.getPrivateKeyInfo() == null) return null;
  ...
  private byte[] getJSONEncodeableFields() {
    ...
    if (privateBackup != null) this.keyval.put("private", privateBackup);

    if (keyPairBackup != null) this.kpr = keyPairBackup;
  ...
  private void encodePem(Writer out, boolean privateKey) {
    ...
    try {
      ...
      else pemWriter.writeObject(new MiscPEMGenerator(this.kpr.getPublicKeyInfo()));
      pemWriter.flush();
    } catch (IOException e) {
      throw new RuntimeException(e.toString());
    }
   ...
   *
   * @return a Signer instance that can be used to sign or verify using RSASSA-PSS
   */
